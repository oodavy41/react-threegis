...
import { threeObj, aniType } from "./object/threeObj";
import { Map } from "react-arcgis";
import meshBuilder from "./tools/threeMesh";
import { tranToArc } from "./tools/arcTools";
OBJLoader(THREE);
...
export default class TScene {
    ...
    setup(context) {
        ...
        geo.applyMatrix(new THREE.Matrix4().makeTranslation(0.5, 0.5, 0));
        ...
        this.choosePlane.position.fromArray(tranToArc(this, [300, 300, 50]));
        ...
        for (var i = 0; i < 5; i++) {
            ...
            mobj.addAnimi(
                aniType.POS,
                tranToArc(this, [Math.random() * this.mapSize, 700 * (i + 1), 0]),
                10 * Math.random() + 10,
                10 * Math.random(),
                true,
                "easeInOutBack",
    ...
    render(context) {
        ...
        this.camera.lookAt(cam.center[0], cam.center[1], cam.center[2]);
    ...
    makeOrigin() {
        ...
        var center = 100;
        cubex.position.fromArray(tranToArc(this, [center + 50, center, 100]));
        this.scene.add(cubex);
        cubey.position.fromArray(tranToArc(this, [center, center + 50, 100]));
        this.scene.add(cubey);
        cubez.position.fromArray(tranToArc(this, [center, center, 150]));
